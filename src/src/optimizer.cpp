...
void SGD::update(Tensor &weights, const Tensor &gradients, double learning_rate) {
    for(size_t i = 0; i < weights.rows(); ++i) {
        for(size_t j = 0; j < weights.cols(); ++j) {
            weights.data_[i][j] -= learning_rate * gradients.data_[i][j];                  // w = w - lr*dw 
        }
    }
}

void SGD::update_biases(Tensor &biases, const Tensor &gradient, double learning_rate) {     // b = b - lr*db
